...
class GPTModel(LLMModel):
    """
    Class to interact with OpenAI API for text generation.
    ...
    def __init__(self, model_name: str, temperature: float = 0.0, max_tokens: int = 4000, top_p: Optional[float] = None):
        ...
        Args:
            model_name (str): Name of the OpenAI model to use (e.g., 'gpt-4', 'gpt-3.5-turbo')
    ...
    def get_batch(self, 
        ...
        if batch.status == 'completed':
        ...
        elif (batch.request_counts.completed == batch.request_counts.total) and (batch.request_counts.total != 0) and (batch.status != 'finalizing') and (batch.output_file_id is not None):
            # Batch is completed, retrieve the output file
            file_response = client.files.content(batch.output_file_id)
            # The output file is a string containing separated JSON objects for each line. Replace the file if it exists
            # return file_response.text
            ...
            file_response = client.files.content(batch.output_file_id)
            if file_response.output_file_id is not None and (batch.request_counts.completed == batch.request_counts.total):
                # The output file is a string containing separated JSON objects for each line. Replace the file if it exists
                # return file_response.text
                with open(batch_response_path, "w") as f:
                    f.write(file_response.text)
                ...
                print("Batch has expired. The output file is available at: ", file_response.output_file_id)
            raise Exception("Batch has expired. Please create a new batch.")        
        else:
